...
import {
  ...
  NameUtils, ProviderSelectionService,
...
export class KayentaStageController implements IComponentController {
  ...
  private initialize = async (): Promise<void> => {
    ...
    this.stage.canaryConfig = {
      storageAccountName: CanarySettings.storageAccountName,
      metricsAccountName: CanarySettings.metricsAccountName,
      ...this.stage.canaryConfig,
      scoreThresholds: {
        marginal: null,
        pass: null,
  ...
  /*
   * Handles a change in canary config. Should not be called on load, since it
   * overrides the score thresholds.
   */
  ...
  public addPair = async (): Promise<void> => {
    ...
    const provider =
      await (has(this.stage, 'deployments.baseline.cloudProvider')
    ...
    try {
      ...
      this.stage.deployments.clusterPairs = [{ control, experiment }];
    } catch(e) {
  ...
  public editServerGroup = async (
    ...
    try {
      const result = await this.$uibModal.open({
        ...
        size: 'lg',
        resolve: {
          title: () => `Configure ${type[0].toUpperCase() + type.substring(1)} Cluster`,
          application: () => this.$scope.application,
      ...
      this.stage.deployments.clusterPairs[index][type] =
